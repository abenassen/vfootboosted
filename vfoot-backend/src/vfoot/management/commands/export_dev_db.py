...
"Comes out empty" is now true, and it was not always: until the coverage guard in
`_per_match_player_totals` existed, the merges that read `MatchShot` and
`MatchAppearance.raw_stats` — tables this export KEEPS — rebuilt a row per
appearance out of the leftovers, and the scorer duly produced a complete, ordered,
entirely believable set of votes in which nobody could exceed 6. The listone LOOKED
right. Anything added here that strips a scoring input must fail loudly like that
guard does, never degrade quietly.

The LISTONE still shows real values, from `vfoot/data/player_ratings_snapshot.json`
(regenerated by `build_player_ratings_snapshot`, versioned in the repo). It travels
with the source rather than with this file, so a collaborator who already has an
older copy gets the values with a plain `git pull` instead of a fresh download.
    ...
    def handle(self, *args, **opts):
        ...
        if not opts["keep_zones"]:
            self.stdout.write(
                ...
                "Il LISTONE invece funziona: i valori arrivano da\n"
                "vfoot/data/player_ratings_snapshot.json, che sta nel repo.\n"
                "Se l'hai appena rigenerato, ricordati di committarlo.")
