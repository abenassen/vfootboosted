  ...
  return (
    <div>
      <div className="flex items-center justify-between text-[11px] font-semibold">
        <span className="text-bad">◄ {homeName}</span>
        ...
        {ordered.map((m) => {
          ...
          return (
            <div key={m.label} className="flex items-center gap-2 text-[11px]" title={`${m.label}: ${m.net.toFixed(3)}`}>
              <span className="w-16 shrink-0 text-ink-soft">{m.label}</span>
              <div className="flex flex-1 items-center">
                <div className="flex h-2.5 flex-1 justify-end overflow-hidden rounded-l-full bg-surface-2">
                  {home ? <div className="h-full bg-bad" style={{ width: `${frac * 100}%` }} /> : null}
                </div>
                <div className="h-3 w-px bg-line" />
                <div className="flex h-2.5 flex-1 overflow-hidden rounded-r-full bg-surface-2">
