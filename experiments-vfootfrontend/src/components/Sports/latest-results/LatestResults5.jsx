const results = [
  {
    ...
    competition: 'SportyBlocks League',
  },
  {
    date: '08/08/2023',
    teams: [
  ...
  },
  {
    ...
    competition: 'World League Tournament',
  },
  {
    date: '07/24/2023',
    teams: [
...
export default function LatestResults5() {
  return (
            ...
            <table className="min-w-full table-auto border-collapse border-spacing-px bg-custom-gray-100 dark:bg-custom-gray-700">
              <thead className="text-xs/tight font-bold text-custom-gray-900 uppercase dark:text-white">
                <tr>
                  ...
                  <th scope="col" className="px-2.5 py-3.5 text-start">
                    <div className="sr-only">Team 1</div>
                  </th>
                  <th scope="col" className="py-3.5 text-center">
                    Score
                  </th>
                  <th scope="col" className="px-2.5 py-3.5 text-start">
                    <div className="sr-only">Team 2</div>
                  </th>
                  <th scope="col" className="py-3.5 ps-1 pe-8 text-start">
                    Competition
                  </th>
                </tr>
              </thead>
              <tbody className="rounded-3xl text-sm/tight font-bold text-custom-gray-900 dark:text-white [&_td]:bg-white dark:[&_td]:bg-custom-gray-800">
                {results.map((event) => (
                  <tr
                    key={event.date}
                    className="border-b border-custom-gray-200 ring-1 first:rounded-t-3xl last:border-0 dark:border-custom-gray-600 [&:first-child>td:first-child]:rounded-ss-3xl [&:first-child>td:last-child]:rounded-se-3xl"
                  >
                    <td className="ps-8 pe-4 text-start">{event.date}</td>
                    <td className="px-2.5 py-4 pe-1 text-end whitespace-nowrap">
                      <div className="flex items-center justify-end gap-2">
                        <div className="min-w-0 text-xs/tight font-bold text-custom-gray-900 dark:text-white">
                          <div className="truncate">{event.teams[0].name}</div>
                        </div>
                        <figure className="relative -my-4 h-12 w-10 overflow-hidden [clip-path:polygon(0_0,_100%_0,_100%_100%,_24%_100%)] md:h-14 md:w-11">
                          <svg
                            viewBox="0 0 420 420"
                            fill="none"
                            xmlns="http://www.w3.org/2000/svg"
                            className={`absolute end-0 top-1/2 aspect-square w-12 -translate-y-1/2 md:w-14 ${event.teams[0].color.primary}`}
                          >
                            <path
                          ...
                          <div
                            className={`absolute inset-0 -z-10 [clip-path:polygon(0_0,_76%_0,_100%_100%,_24%_100%)] ${event.teams[0].color.secondary}`}
                          ></div>
                        </figure>
                      </div>
                    </td>
                    <td className="py-4 text-center">{event.outcome.score}</td>
                    <td className="px-2.5 py-4 ps-1 text-start whitespace-nowrap">
                      <div className="flex flex-row-reverse items-center justify-end gap-2">
                        <div className="min-w-0 text-xs/tight font-bold text-custom-gray-900 dark:text-white">
                          <div className="truncate">{event.teams[1].name}</div>
                        </div>
                        <figure className="relative -my-4 h-12 w-10 overflow-hidden [clip-path:polygon(0_0,_100%_0,_76%_100%,_0_100%)] md:h-14 md:w-11">
                          <svg
                            viewBox="0 0 420 420"
                            fill="none"
                            xmlns="http://www.w3.org/2000/svg"
                            className={`absolute start-0 top-1/2 aspect-square w-12 -translate-y-1/2 md:w-14 ${event.teams[1].color.primary}`}
                          >
                            <path
                              d="M201.646 416.137C144.946 389.951 97.469 343.545 60.543 278.221C30.33 224.771 13.58 169.737 4.849 132.979L0 112.558L20.478 108.517C29.676 106.701 36.353 98.519 36.353 89.064C36.353 87.535 36.171 85.986 35.811 84.46L31.579 64.862L68.813 56.045V18.129L83.947 14.518C125.355 4.884 167.706 0 210.202 0C252.699 0 294.762 4.884 336.17 14.518L351.208 18.129V56.045L388.444 64.862L384.015 84.461C383.657 85.986 383.572 87.538 383.572 89.064C383.572 98.519 390.297 106.701 399.497 108.517L420 112.558L415.161 132.981C406.428 169.739 389.684 224.774 359.473 278.221C322.549 343.545 275.075 389.95 218.367 416.141L210.01 420L201.646 416.137Z"
                              fill="currentColor"
                          ...
                          <div
                            className={`absolute inset-0 -z-10 [clip-path:polygon(24%_0,_100%_0,_76%_100%,_0_100%)] ${event.teams[1].color.secondary}`}
                          ></div>
                        </figure>
                      </div>
                    </td>
                    <td className="py-4 ps-1 pe-8 text-start whitespace-nowrap">{event.competition}</td>
                  </tr>
                ))}
              </tbody>
            </table>
