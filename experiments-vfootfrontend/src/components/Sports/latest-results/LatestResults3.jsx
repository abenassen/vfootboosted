...
export default function LatestResults3() {
  return (
    <div className="mx-auto w-full px-5 sm:w-[380px]">
      <div className="rounded-3xl border border-custom-gray-200 bg-white px-8 py-7 dark:border-custom-gray-600 dark:bg-custom-gray-800">
        <div className="relative -mx-8 -mt-7 px-8 py-7">
          <h3 className="pe-10 text-base/tight font-bold text-custom-gray-900 dark:text-white">Golden Team Last 5</h3>
            ...
            >
              ...
              <div className="relative rounded-t-3xl bg-white ring-1 ring-custom-gray-200 after:absolute after:-bottom-[1px] after:h-px after:w-full after:bg-white dark:bg-custom-gray-800 dark:ring-custom-gray-600 dark:after:bg-custom-gray-800">
                {result.events.map((event) => (
                  <div
                    key={event.id}
                    className="flex items-center justify-between gap-x-2 border-b border-custom-gray-200 px-8 py-4 last:border-b-0 dark:border-custom-gray-600"
                  >
                    <div className="flex flex-1 items-center gap-2">
                      <div className="basis-5 text-xs/tight font-bold text-custom-gray-900 uppercase dark:text-white">{event.home ? 'vs' : '@'}</div>
                      <figure className="relative -my-4 h-12 w-10 shrink-0 overflow-hidden [clip-path:polygon(0_0,_100%_0,_100%_100%,_24%_100%)] md:h-14 md:w-11">
                        <svg
                          viewBox="0 0 420 420"
                          fill="none"
                          xmlns="http://www.w3.org/2000/svg"
                          className={`absolute end-0 top-1/2 aspect-square w-12 -translate-y-1/2 md:w-14 ${event.team.color.primary}`}
                        >
                          <path
                            d="M201.646 416.137C144.946 389.951 97.469 343.545 60.543 278.221C30.33 224.771 13.58 169.737 4.849 132.979L0 112.558L20.478 108.517C29.676 106.701 36.353 98.519 36.353 89.064C36.353 87.535 36.171 85.986 35.811 84.46L31.579 64.862L68.813 56.045V18.129L83.947 14.518C125.355 4.884 167.706 0 210.202 0C252.699 0 294.762 4.884 336.17 14.518L351.208 18.129V56.045L388.444 64.862L384.015 84.461C383.657 85.986 383.572 87.538 383.572 89.064C383.572 98.519 390.297 106.701 399.497 108.517L420 112.558L415.161 132.981C406.428 169.739 389.684 224.774 359.473 278.221C322.549 343.545 275.075 389.95 218.367 416.141L210.01 420L201.646 416.137Z"
                            fill="currentColor"
                        ...
                        <div
                          className={`absolute inset-0 -z-10 [clip-path:polygon(0_0,_76%_0,_100%_100%,_24%_100%)] ${event.team.color.secondary}`}
                        ></div>
                      </figure>
                      <div className="ms-1 min-w-0 text-xs/tight font-bold text-custom-gray-900 dark:text-white">
                        <div className="truncate">{event.team.name}</div>
                      </div>
                    </div>

                    <div className="inline-flex gap-1.5 text-xs/tight font-bold text-custom-gray-900 dark:text-white">
                      {event.outcome === 'win' && <span className="text-custom-green">W</span>}
                      {event.outcome === 'loss' && <span className="text-custom-red">L</span>}
                      {event.outcome === 'draw' && <span className="text-custom-gray">D</span>}
                      {event.score}
                    </div>
                  </div>
                ))}
