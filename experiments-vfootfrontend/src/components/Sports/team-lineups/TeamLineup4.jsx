...
export default function TeamLineup4() {
  return (
    <div className="mx-auto w-full lg:w-[700px]">
      <div className="rounded-3xl border border-custom-gray-200 bg-white px-8 py-7 dark:border-custom-gray-600 dark:bg-custom-gray-800">
        <div className="relative -mx-8 -mt-7 px-8 py-7">
          <h3 className="pe-10 text-base/tight font-bold text-custom-gray-900 dark:text-white">Team Lineup V4</h3>
        ...
        <div className="-mx-8 -mb-7 overflow-x-auto rounded-3xl bg-white ring-1 ring-custom-gray-200 dark:bg-custom-gray-800 dark:ring-custom-gray-600">
          <div className="grid grid-cols-[auto_auto_1fr_auto_auto_auto_auto] gap-x-2.5">
            {/* Heading */}
            <div className="col-span-full -mb-6 grid grid-cols-subgrid bg-custom-gray-100 pb-6 text-xs/tight font-bold text-custom-gray-900 uppercase dark:bg-custom-gray-700 dark:text-white">
              ...
              <div className="min-w-[3.125rem] py-4 text-end">Sog</div>
              <div className="min-w-[3.125rem] py-4 text-end">Fou</div>
              <div className="min-w-[3.75rem] py-4 ps-4 pe-8 text-end">Mins</div>
            </div>
            {/* Heading / End */}
            ...
            {lineup.map((group, groupIndex) => (
              <div key={group.id} className="col-span-full grid grid-cols-subgrid">
                ...
                {group.players.map((player) => (
                  ...
                  >
                    <div className="py-5 ps-8 pe-2.5 text-custom-gray dark:text-custom-gray-400">{player.number}</div>

                    <div className="py-5 font-bold">{player.position}</div>

                    <div>
                      <div className="flex min-w-[12rem] items-center gap-0.5">
                        {player.img ? (
                          <img src={player.img} alt={player.name} className="mt-0.5 h-14 w-20 shrink-0 object-cover object-top" />
                        ) : (
                          <>
                            <img
                              src="https://danfisher-bucket-1.s3.us-east-2.amazonaws.com/sportyblocks/player-placeholder.png"
                              alt={player.name}
                              className="mt-0.5 h-14 w-20 shrink-0 object-cover object-top dark:hidden"
                            />
                            <img
                              src="https://danfisher-bucket-1.s3.us-east-2.amazonaws.com/sportyblocks/player-placeholder-white.png"
                              alt={player.name}
                              className="mt-0.5 hidden h-14 w-20 shrink-0 object-cover object-top dark:block"
                            />
                          </>
                        )}
                    ...
                    <div className="py-5 text-end font-bold">{player.sog}</div>
                    <div className="py-5 text-end font-bold">{player.fou}</div>
                    <div className="py-5 pe-8 text-end font-bold">{player.mins}’</div>
                  </div>
                ))}
                {/* Players / End */}
              </div>
