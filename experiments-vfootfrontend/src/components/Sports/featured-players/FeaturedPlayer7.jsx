...
export default function FeaturedPlayer7() {
  return (
    <div className='mx-auto w-[500px] px-5'>
      <div className='@container space-y-3 text-custom-gray-900 dark:text-white'>
        {players.map((player) => (
          <div
            key={player.name}
            className='@container overflow-hidden rounded-3xl border border-custom-gray-200 bg-white dark:border-custom-gray-600 dark:bg-custom-gray-800'
          >
            <div className='flex items-center'>
              <div className='relative isolate flex flex-1 items-center overflow-hidden'>
                <div className='h-[98px] w-[140px] overflow-hidden'>
                  <img src={player.img} alt={player.name} className='-mt-2 w-40 max-w-none object-cover object-top' />
                </div>
                <svg
                  viewBox='0 0 420 420'
                  fill='none'
                  xmlns='http://www.w3.org/2000/svg'
                  className={`absolute -start-2.5 top-1/2 -z-10 h-20 w-20 -translate-y-1/2 ${player.team.color}`}
                >
                  <path
                ...
                <div
                  className={`absolute inset-y-0 start-0 -z-20 w-[120px] [clip-path:polygon(0_0,74%_0,100%_100%,0%_100%)] ${player.team.bgColor}`}
                ></div>
                <div>
                  <div className='text-sm/tight font-bold @md:text-base/tight'>{player.name}</div>
                  <div className='text-xs/tight'>{player.position}</div>
                </div>
              </div>
              <div className='py-6 pe-6 text-end @md:py-8 @md:pe-8'>
                <div className='text-sm/tight font-bold @md:text-base/tight'>{player.stat.value}</div>
                <div className='text-xs/tight'>{player.stat.label}</div>
              </div>
            </div>
          </div>
        ))}
