...
export default function FeaturedPlayer5() {
  return (
    <div className="mx-auto w-[500px] px-5">
      <div className="@container overflow-hidden rounded-3xl border border-custom-gray-200 bg-white dark:border-custom-gray-600 dark:bg-custom-gray-800">
        <div className="px-6 py-6 @md:px-8 @md:py-7">
          <h3 className="text-base/tight font-bold text-custom-gray-900 dark:text-white">Featured Player V5</h3>
        </div>

        <div className="inline-block min-w-full align-middle">
          <table className="min-w-full table-auto border-collapse border-spacing-px rounded-3xl bg-custom-gray-100 ring-1 ring-custom-gray-200 dark:bg-custom-gray-700 dark:ring-custom-gray-600">
            <thead className="text-xs/tight font-bold text-custom-gray-900 uppercase dark:text-white">
              <tr>
                <th scope="col" className="w-48 py-3.5 ps-6 pe-3 text-start @md:ps-8">
                  Team and Player
                </th>
                <th scope="col" className="px-2 py-3.5 text-end @md:px-3">
                  G
                </th>
                <th scope="col" className="py-3.5 ps-2 pe-6 text-end @md:ps-3 @md:pe-8">
                  A
                </th>
              </tr>
            </thead>
            <tbody className="rounded-3xl text-sm/tight font-bold text-custom-gray-900 dark:text-white [&_td]:bg-white dark:[&_td]:bg-custom-gray-800">
              {players.map((player) => (
                ...
                >
                  <td className="w-2/3 ps-6 pe-2.5 @md:ps-8">
                    <div className="flex items-center gap-1">
                      <div className="me-1 @md:me-2.5">
                        <svg
                          viewBox="0 0 420 420"
                          fill="none"
                          xmlns="http://www.w3.org/2000/svg"
                          className={`h-5 w-5 @md:h-6 @md:w-6 ${player.team.color}`}
                        >
                          <path
                      ...
                      <div>
                        <div className="relative isolate -my-px h-14 w-16 overflow-hidden @md:h-[60px] @md:w-20">
                          <img src={player.img} alt={player.name} className="-ms-2 w-20 max-w-none object-cover object-top @md:w-24" />
                          <div
                            className={`absolute inset-y-0 start-0 -z-10 w-14 [clip-path:polygon(0_0,74%_0,100%_100%,26%_100%)] ${player.team.bgColor}`}
                          ></div>
                        </div>
                      </div>
                      <div>
                        <div className="text-xs/tight @md:text-sm/tight">{player.name}</div>
                        <div className="text-[0.6875rem]/tight font-normal @md:text-xs/tight">{player.position}</div>
                      </div>
                    </div>
                  </td>
                  {player.stats.map((stat) => (
                    <td
                      key={stat.label}
                      className="w-10 px-2 py-5 text-end text-xs/tight last:ps-0 last:pe-6 @md:w-12 @md:px-3 @md:text-sm/tight @md:last:pe-8"
                    >
                      {stat.value}
                    </td>
                  ))}
