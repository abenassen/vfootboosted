    ...
    players: [
      {
        ...
        team: {
          ...
          bgColor: 'bg-team-golden-secondary',
        },
        statValue: '396’',
      },
      {
        ...
        team: {
          ...
          bgColor: 'bg-team-emerald-secondary',
        },
        statValue: '311’',
      },
    ],
...
export default function FeaturedPlayer6() {
  return (
    <div className="mx-auto w-[500px] px-5">
      <div className="@container overflow-hidden rounded-3xl border border-custom-gray-200 bg-white dark:border-custom-gray-600 dark:bg-custom-gray-800">
        <div className="px-6 py-6 @md:px-8 @md:py-7">
          <h3 className="text-base/tight font-bold text-custom-gray-900 dark:text-white">Featured Player V6</h3>
        </div>

        <div>
          {data.map((row) => (
            <div key={row.title} className="overflow-hidden rounded-t-3xl">
              <div className="bg-custom-blue px-6 pt-4 pb-10 @md:px-8">
                <h4 className="text-xs/tight font-bold text-white uppercase">{row.title}</h4>
              </div>
              <div className="-mt-6 overflow-hidden rounded-t-3xl ring-1 ring-custom-gray-200 dark:ring-custom-gray-600">
                <div className="flex justify-between bg-custom-gray-100 px-6 pt-4 pb-10 text-xs/tight font-bold text-custom-gray-900 uppercase @md:px-8 dark:bg-custom-gray-700 dark:text-white">
                  <div>Team and Player</div>
                  <div>{row.stat}</div>
                </div>

                <div className="-mx-px -mt-6 rounded-t-3xl border-x border-t border-custom-gray-200 bg-white dark:border-custom-gray-600 dark:bg-custom-gray-800">
                  {row.players.map((player) => (
                    <div
                      key={player.name}
                      className="flex items-center justify-between border-b border-custom-gray-200 px-6 last:border-b-0 @md:px-8 dark:border-custom-gray-600"
                    >
                      <div className="flex gap-2">
                        <div className="relative h-[60px] w-[46px] overflow-hidden [clip-path:polygon(0_0,_100%_0,_100%_100%,_24%_100%)]">
                          <svg
                            viewBox="0 0 420 420"
                            fill="none"
                            xmlns="http://www.w3.org/2000/svg"
                            className={`absolute end-0 top-1/2 aspect-square w-[60px] -translate-y-1/2 ${player.team.color}`}
                          >
                            <path
                          ...
                          <div className={`absolute inset-0 -z-10 [clip-path:polygon(0_0,_76%_0,_100%_100%,_24%_100%)] ${player.team.bgColor}`}></div>
                        </div>
                        <div className="flex items-center gap-2">
                          <figure className="h-10 w-10 overflow-hidden rounded-full border border-custom-gray-200 bg-custom-gray-100 dark:border-custom-gray-600 dark:bg-custom-gray-700">
                            <img src={player.img} alt={player.name} className="-ms-2 w-14 max-w-none object-cover object-top" />
                          </figure>
                          <div>
                            <div className="text-sm/tight font-bold text-custom-gray-900 dark:text-white">{player.name}</div>
                          </div>
                        </div>
                      </div>
                      <div className="text-sm/tight font-bold text-custom-gray-900 dark:text-white">{player.statValue}</div>
                    </div>
                  ))}
                </div>
              </div>
