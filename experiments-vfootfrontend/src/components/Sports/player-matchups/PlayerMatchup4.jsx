...
export default function PlayerMatchup4() {
  return (
    <div className="mx-auto w-full max-w-[460px]">
      <div className="@container rounded-3xl border border-custom-gray-200 bg-white text-gray-800 dark:border-custom-gray-600 dark:bg-custom-gray-800 dark:text-white">
        <div className="relative px-5 py-7 @sm:px-8">
          <h3 className="text-base/tight font-bold">Players Matchup V4</h3>
        </div>
        <div className="overflow-hidden rounded-3xl px-6 ring-1 ring-custom-gray-200 @sm:px-8 dark:ring-custom-gray-600">
          <div className="relative -mx-6 flex items-center justify-between gap-x-1 bg-custom-gray-100 @sm:-mx-8 dark:bg-custom-gray-700">
            <div className="flex items-baseline gap-x-1 px-6 @sm:px-8">
              {data.players.map((player, index) => (
                <div key={player.name} className="flex items-baseline gap-x-1 text-sm/tight font-bold">
                  {player.name}
                  {index === 0 && <span className="text-[0.625rem]/tight uppercase">vs</span>}
                </div>
              ))}
            </div>
            <div className="flex gap-x-2 pe-6 @sm:pe-8">
              {data.players.map((player) => (
                <div key={player.name} className="group relative isolate flex h-[60px] w-20 flex-1 justify-center gap-x-3 overflow-hidden">
                  <figure className="relative w-24 overflow-hidden object-center">
                    <img src={player.img} alt={player.name} className="absolute start-1/2 top-0 w-24 max-w-none -translate-x-1/2" />
                  </figure>
                  <svg
                    viewBox="0 0 420 420"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                    className={`absolute end-0 top-1/2 -z-10 h-7 w-7 -translate-y-1/2 ${player.team.color.primary}`}
                  >
                    <path
                  ...
                  <div
                    className={`absolute inset-y-0 end-0 -z-20 w-14 [clip-path:polygon(24%_0,_100%_0,_76%_100%,_0_100%)] ${player.team.bgColor}`}
                  ></div>
                </div>
              ))}
            </div>
          ...
          <div className="-mx-6 divide-y divide-custom-gray-200 border-t border-custom-gray-200 @sm:-mx-8 dark:divide-custom-gray-600 dark:border-custom-gray-600">
            {data.stats.map((stat) => (
              <div key={stat.name} className="px-6 py-5 @sm:px-8">
                <div className="grid grid-cols-[1fr_auto_auto] gap-x-3 @sm:gap-x-4">
                  <div className="truncate text-sm/tight">{stat.name}</div>
                  <div className="w-20 text-end text-sm/tight font-bold">{stat.values.player1.value}</div>
                  <div className="w-20 text-end text-sm/tight font-bold">{stat.values.player2.value}</div>
                </div>
              </div>
            ))}
          </div>
