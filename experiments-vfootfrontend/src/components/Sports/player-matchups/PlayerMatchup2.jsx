...
export default function PlayerMatchup2() {
  return (
    <div className="mx-auto w-full max-w-[460px]">
      <div className="@container rounded-3xl border border-custom-gray-200 bg-white text-gray-800 dark:border-custom-gray-600 dark:bg-custom-gray-800 dark:text-white">
        <div className="relative px-16 py-7">
          <h3 className="text-center text-base/tight font-bold">Players Matchup V2</h3>
          <div>
        ...
        <div className="overflow-hidden rounded-3xl px-6 ring-1 ring-custom-gray-200 @sm:px-8 dark:ring-custom-gray-600">
          <div className="relative">
            <div className="relative -mx-6 flex items-center justify-between border-b border-custom-gray-200 @sm:-mx-8 dark:border-custom-gray-600">
              {data.players.map((player) => (
                <div key={player.name} className="group relative isolate flex h-20 flex-1 items-center justify-center gap-x-3">
                  <figure className="relative h-20 w-36 overflow-hidden object-center">
                    <img src={player.img} alt={player.name} className="absolute start-1/2 -top-2 w-full max-w-none -translate-x-1/2" />
                  </figure>
                  <div className={`absolute inset-y-0 -z-10 w-1/2 overflow-hidden group-first:start-0 group-last:end-0 ${player.team.bgColor}`}>
                    <div className="-indent-1 text-[80px]/snug font-extrabold tracking-tighter text-white/40 italic mix-blend-overlay">
            ...
            <div className="absolute start-1/2 top-1/2 flex h-10 w-10 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-xl bg-custom-gray-900 text-sm/tight font-bold text-white uppercase">
              vs
            </div>
          </div>
          <div className="relative flex items-center justify-between gap-x-10 py-4">
            {data.players.map((player) => (
          ...
          <div className="-mx-6 divide-y divide-custom-gray-200 border-t border-custom-gray-200 @sm:-mx-8 dark:divide-custom-gray-600 dark:border-custom-gray-600">
            {data.stats.map((stat) => (
              <div key={stat.name} className="grid grid-cols-[auto_1fr_auto] items-center gap-x-3 px-6 py-5 @sm:gap-x-4 @sm:px-8">
                <div
                  className={classNames(
                    stat.values.player1.isActive ? 'bg-team-golden-primary text-white' : 'bg-custom-gray-100 dark:bg-custom-gray-700',
                    'inline-flex h-10 w-10 items-center justify-center rounded-xl text-sm/tight font-bold',
                  )}
                >
                  {stat.values.player1.value}
                </div>
                <div className="truncate px-5 text-center text-sm/tight">{stat.name}</div>
                <div
                  className={classNames(
                    stat.values.player2.isActive ? 'bg-team-emerald-primary text-white' : 'bg-custom-gray-100 dark:bg-custom-gray-700',
                    'inline-flex h-10 w-10 items-center justify-center rounded-xl text-sm/tight font-bold',
                  )}
                >
                  {stat.values.player2.value}
                </div>
              </div>
            ))}
          </div>
