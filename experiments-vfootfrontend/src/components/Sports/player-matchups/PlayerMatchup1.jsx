...
export default function PlayerMatchup1() {
  return (
    <div className="mx-auto w-full max-w-[460px]">
      <div className="rounded-3xl border border-custom-gray-200 bg-white text-gray-800 dark:border-custom-gray-600 dark:bg-custom-gray-800 dark:text-white">
        <div className="relative px-16 py-7">
          <h3 className="text-center text-base/tight font-bold">Players Matchup V1</h3>
          <div>
        ...
        <div className="rounded-3xl px-6 ring-1 ring-custom-gray-200 @lg:px-8 dark:ring-custom-gray-600">
          <div className="relative flex items-center justify-between gap-x-10 py-2.5">
            <div className="absolute start-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 text-sm/tight font-bold uppercase">vs</div>
            {data.players.map((player) => (
              <div key={player.name} className="group flex items-center gap-x-3 last:flex-row-reverse">
                <figure className="relative aspect-square w-10 shrink-0 overflow-hidden rounded-full border border-custom-gray-200 bg-custom-gray-100 dark:border-custom-gray-600 dark:bg-custom-gray-700">
                  <img src={player.img} alt={player.name} className="absolute start-1/2 w-16 max-w-none -translate-x-1/2" />
                </figure>
                <div className="group-last:text-end">
                  <h4 className="text-sm/tight font-bold">{player.name}</h4>
                  <div className="text-xs/tight">{player.position}</div>
                </div>
              </div>
            ))}
          </div>
          <div className="-mx-6 divide-y divide-custom-gray-200 border-t border-custom-gray-200 @lg:-mx-8 dark:divide-custom-gray-600 dark:border-custom-gray-600">
            {data.stats.map((stat) => (
              <div key={stat.name} className="grid grid-cols-[auto_1fr_5.5rem_1fr_auto] items-center gap-x-3 px-6 py-5 @lg:gap-x-4 @lg:px-8">
                <div className="min-w-[1.5rem] text-sm/tight font-bold">{stat.values.player1.value}</div>
                <div>
                  <div className="flex h-1 flex-row-reverse overflow-hidden rounded-full bg-custom-gray-200 dark:bg-custom-gray-600">
                    <div className="h-full rounded-full bg-team-golden-primary" style={{ width: stat.values.player1.percentage }}></div>
                  </div>
                </div>
                ...
                <div>
                  <div className="flex h-1 overflow-hidden rounded-full bg-custom-gray-200 dark:bg-custom-gray-600">
                    <div className="h-full rounded-full bg-team-emerald-primary" style={{ width: stat.values.player2.percentage }}></div>
                  </div>
                </div>
                <div className="min-w-[1.5rem] text-end text-sm/tight font-bold">{stat.values.player2.value}</div>
              </div>
            ))}
          </div>
        </div>
