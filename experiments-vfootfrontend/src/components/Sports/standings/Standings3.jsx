...
export default function Standings3() {
  return (
    <div className="mx-auto w-[340px] sm:w-[460px]">
        ...
        <div className="-mx-8 -mb-7 overflow-x-auto rounded-3xl bg-white ring-1 ring-custom-gray-200 dark:bg-custom-gray-800 dark:ring-custom-gray-600">
          <div className="inline-block min-w-full align-middle">
            {standings.map((group, groupIndex) => (
              <table
                key={group.title}
                className={`min-w-full table-auto border-collapse border-spacing-px rounded-3xl ${
                  groupIndex === 0 ? 'bg-custom-blue' : 'bg-team-emerald-primary'
                }`}
              >
                <thead className="text-xs/tight font-bold text-white uppercase">
                  <tr>
                ...
                <tbody className="rounded-3xl text-sm/tight font-bold text-custom-gray-900 dark:text-white">
                  {group.teams.map((team) => (
                    <tr
                      key={team.name}
                      className="border-b border-custom-gray-200 first:rounded-t-3xl last:border-0 dark:border-custom-gray-600 [&_td]:bg-white dark:[&_td]:bg-custom-gray-800 [&:first-child>td:first-child]:rounded-ss-3xl [&:first-child>td:last-child]:rounded-se-3xl [&:nth-child(even)>td]:bg-custom-gray-100 dark:[&:nth-child(even)>td]:bg-custom-gray-700"
                    >
                      <td className="py-4 ps-8 pe-4 text-start whitespace-nowrap">
                        <div className="flex items-center gap-1.5">
                          <div>{team.pos}</div>
                          <svg viewBox="0 0 420 420" fill="none" xmlns="http://www.w3.org/2000/svg" className={`h-6 w-6 shrink-0 ${team.color}`}>
                            <path
                          ...
                          <div className="w-24 min-w-0 flex-1 truncate">{team.name}</div>
                        </div>
                      </td>
                      ...
                      <td className="py-4 ps-1 pe-8 text-end font-normal whitespace-nowrap text-custom-gray dark:text-custom-gray-400">
                        {team.stats.diff}
                      </td>
                    </tr>
                  ))}
