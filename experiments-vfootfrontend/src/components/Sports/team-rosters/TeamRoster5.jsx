...
export default function TeamRoster5() {
  return (
    <div className="mx-auto w-full px-5 xl:w-[1100px]">
      <div className="rounded-3xl border border-custom-gray-200 bg-white px-8 py-7 dark:border-custom-gray-600 dark:bg-custom-gray-800">
        <div className="relative -mx-8 -mt-7 px-8 py-7">
          <h3 className="pe-10 text-base/tight font-bold text-custom-gray-900 dark:text-white">Team Roster V5</h3>
          ...
          <div className="grid grid-cols-[1fr_auto_auto_auto_auto_auto_auto_auto_auto_auto] gap-x-2.5">
            ...
            {roster.map((group, groupIndex) => (
              <div key={group.id} className="col-span-full grid grid-cols-subgrid">
                {group.title && (
                  <div
                    className={`col-span-full -mb-6 rounded-t-3xl px-8 pt-4 pb-10 text-xs/tight font-bold text-white uppercase ring-1 ring-custom-gray-200 dark:ring-custom-gray-600 [&+div]:rounded-t-3xl [&+div]:ring-1 [&+div]:ring-custom-gray-200 dark:[&+div]:ring-custom-gray-600 [&+div>div]:rounded-t-3xl ${
                ...
                {group.players.map((player) => (
                  <div
                    key={player.id}
                    className="group col-span-full grid grid-cols-subgrid items-center border-b border-custom-gray-200 bg-white text-sm/tight first:rounded-t-3xl first:ring-1 first:ring-custom-gray-200 last:border-b-0 dark:border-custom-gray-600 dark:bg-custom-gray-800 dark:first:ring-custom-gray-600"
                  >
                    <div className="flex min-w-[17.5rem] gap-3 overflow-hidden group-first:rounded-ss-3xl group-last:rounded-es-3xl">
                      <div className={`relative isolate flex min-w-[11.25rem] shrink-0 items-center gap-2 ${player.bgColor}`}>
                        <div className="relative flex shrink-0 items-center ps-6 font-bold text-white uppercase">
                          {player.number ? player.number : <span className="min-w-[1.25rem]">-</span>}
                        </div>
                        <figure className="relative h-14 w-20 shrink-0">
                          {player.img ? (
                            <img src={player.img} alt={player.name} className="relative mt-0.5 h-full w-full object-cover object-top" />
                          ) : (
                            <>
                              <img
                                src="https://danfisher-bucket-1.s3.us-east-2.amazonaws.com/sportyblocks/player-placeholder.png"
                                alt={player.name}
                                className="relative mt-0.5 h-full w-full object-cover object-top"
                              />
                            </>
                          )}
                          <svg
                            ...
                            className="absolute start-1/2 top-2.5 -z-10 h-20 w-20 -translate-x-1/2 text-team-golden-primary"
                          >
                            <path
                              d="M201.646 416.137C144.946 389.951 97.469 343.545 60.543 278.221C30.33 224.771 13.58 169.737 4.849 132.979L0 112.558L20.478 108.517C29.676 106.701 36.353 98.519 36.353 89.064C36.353 87.535 36.171 85.986 35.811 84.46L31.579 64.862L68.813 56.045V18.129L83.947 14.518C125.355 4.884 167.706 0 210.202 0C252.699 0 294.762 4.884 336.17 14.518L351.208 18.129V56.045L388.444 64.862L384.015 84.461C383.657 85.986 383.572 87.538 383.572 89.064C383.572 98.519 390.297 106.701 399.497 108.517L420 112.558L415.161 132.981C406.428 169.739 389.684 224.774 359.473 278.221C322.549 343.545 275.075 389.95 218.367 416.141L210.01 420L201.646 416.137Z"
                              fill="currentColor"
                        ...
                        <div className="font-bold text-white uppercase">{player.position}</div>
                      </div>
                      <div className="self-center font-bold">
                        <div className="text-xs/tight">{player.firstName}</div>
