...
export default function TeamRoster2() {
  return (
    <div className="mx-auto w-full px-5 xl:w-[1100px]">
      <div className="rounded-3xl border border-custom-gray-200 bg-white px-8 py-7 dark:border-custom-gray-600 dark:bg-custom-gray-800">
        <div className="relative -mx-8 -mt-7 px-8 py-7">
          <h3 className="pe-10 text-base/tight font-bold text-custom-gray-900 dark:text-white">Team Roster V2</h3>
            ...
            {roster.map((group, groupIndex) => (
              <div key={group.id} className="col-span-full grid grid-cols-subgrid">
                ...
                {group.players.map((player) => (
                  ...
                  >
                    <div className="min-w-[14rem] overflow-hidden group-first:rounded-ss-3xl group-last:rounded-es-3xl">
                      <div className="relative isolate flex items-center gap-2">
                        <div
                          className={`relative -me-9 flex w-[5.625rem] shrink-0 items-center self-stretch ps-8 pe-2 font-bold text-white uppercase before:absolute before:inset-0 before:-z-10 ${player.bgColor}`}
                        >
                          {player.position}
                        </div>
                        {player.img ? (
                          <img src={player.img} alt={player.name} className="relative mt-0.5 h-14 w-14 shrink-0 object-cover object-top" />
                        ) : (
                          <>
                            <img
                              src="https://danfisher-bucket-1.s3.us-east-2.amazonaws.com/sportyblocks/player-placeholder.png"
                              alt={player.name}
                              className="relative mt-0.5 h-14 w-14 shrink-0 object-cover object-top dark:hidden"
                            />
                            <img
                              src="https://danfisher-bucket-1.s3.us-east-2.amazonaws.com/sportyblocks/player-placeholder-white.png"
                              alt={player.name}
                              className="relative mt-0.5 hidden h-14 w-14 shrink-0 object-cover object-top dark:block"
                            />
                          </>
                        )}
