...
export default function MatchSchedule6() {
  return (
    <div className="mx-auto w-full max-w-[340px]">
      {data.map((match) => (
        ...
        >
          <div className="relative isolate overflow-hidden rounded-3xl bg-white ring-1 ring-custom-gray-200 dark:bg-custom-gray-800 dark:ring-custom-gray-600">
            <div className="flex items-center justify-between">
              {match.teams.map((team) => (
                <div key={team.name} className="flex gap-x-4 last:flex-row-reverse">
                  <div className={`w-8 ${team.bgColor}`}></div>
                  <div className="flex flex-col items-center gap-2 py-5">
                    <svg viewBox="0 0 420 420" fill="none" xmlns="http://www.w3.org/2000/svg" className={`h-6 w-6 ${team.color.primary}`}>
                      <path
                    ...
                    <div className="text-sm/tight font-bold uppercase">{team.abbr}</div>
                  </div>
                  <div className="flex min-w-[32px] items-center justify-center text-xl/tight font-bold">
                    <span>-</span>
                  </div>
                </div>
              ))}
            ...
            <div className="absolute start-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 text-center text-sm/tight">
              <div className="text-2xl/none font-bold">{match.date.day}</div>
              <div className="flex gap-0.5 text-xs/tight uppercase">
                <span className="font-bold">{match.date.month}</span>
                <span>{match.date.year}</span>
              </div>
            </div>
          </div>
