const data = [
  {
    ...
    venue: {
      name: 'Boiling Rock Arena',
    },
    league: 'World League Tournament',
...
export default function MatchSchedule2() {
  return (
    <div className="mx-auto w-full max-w-[340px]">
      {data.map((match) => (
        <div
          key={match.id}
          className="@container rounded-3xl border border-custom-gray-200 bg-custom-gray-100 text-gray-800 dark:border-custom-gray-600 dark:bg-custom-gray-700 dark:text-white"
        >
          <div className="rounded-3xl bg-white dark:bg-custom-gray-800">
            <div className="relative mb-px overflow-hidden rounded-3xl ring-1 ring-custom-gray-200 dark:ring-custom-gray-600">
              <div className="flex justify-between gap-x-24">
                {match.teams.map((team) => (
                  <div key={team.name} className="group relative isolate flex-1 pt-6 pb-6">
                    <div className="flex flex-col items-center gap-2 text-center group-first:ps-5 group-last:pe-5">
                      <div className="relative isolate">
                        <svg
                          viewBox="0 0 420 420"
                          fill="none"
                          xmlns="http://www.w3.org/2000/svg"
                          className={`h-[60px] w-[60px] ${team.color.primary}`}
                        >
                          <path
                        ...
                        <svg
                          viewBox="0 0 70 70"
                          xmlns="http://www.w3.org/2000/svg"
                          className="absolute start-1/2 top-1/2 -z-10 h-[70px] w-[70px] -translate-x-1/2 -translate-y-1/2 fill-white dark:fill-custom-gray-800"
                        >
                          <path d="M33.6073 69.3557C24.1573 64.9914 16.2443 57.2574 10.0903 46.3698C5.055 37.4622 2.26333 28.2895 0.808 22.1632L0 18.7596L3.413 18.0862C4.946 17.7832 6.05867 16.4199 6.05867 14.8439C6.05867 14.5889 6.02833 14.3306 5.96833 14.0766L5.26333 10.8103L11.4687 9.34062V3.02165L13.991 2.41966C20.8927 0.813663 27.951 0 35.0337 0C42.1167 0 49.127 0.813663 56.0283 2.41966L58.5347 3.02165V9.34095L64.7407 10.8106L64.0027 14.0773C63.943 14.3313 63.9287 14.5899 63.9287 14.8443C63.9287 16.4203 65.0493 17.7836 66.5827 18.0866L70 18.7596L69.1933 22.1636C67.738 28.2895 64.9473 37.4625 59.9117 46.3701C53.758 57.2577 45.8457 64.9914 36.3943 69.3567L35.0013 70L33.6073 69.3557Z" />
                        </svg>
                      </div>
                      <div className="text-xs/tight font-bold">{team.abbr}</div>
                    </div>
                    <div
                      className={`absolute top-0 -z-10 h-24 w-28 group-first:start-0 group-first:[clip-path:polygon(0_0,0_100%,100%_0)] group-last:end-0 group-last:[clip-path:polygon(100%_0,0_0,100%_100%)] ${team.color.secondary}`}
                    ></div>
                  </div>
                ))}
              </div>
              <div className="absolute start-1/2 bottom-0 flex w-24 -translate-x-1/2 flex-col gap-y-2">
                <div className="text-center text-[1.75rem]/tight font-bold">VS</div>
                <div className="rounded-t-2-5xl border border-b-0 border-custom-gray-200 py-2.5 text-center text-xs/tight dark:border-custom-gray-600">
                  <div className="font-bold">{match.time.start}</div>
                  <div>{match.time.zone}</div>
                </div>
              </div>
            </div>
            <div className="flex justify-between rounded-b-3xl border-b border-custom-gray-200 bg-white px-8 py-4 text-xs/tight dark:border-custom-gray-600 dark:bg-custom-gray-800">
              <div>
                <div className="font-bold">{match.venue.name}</div>
                <div>Venue</div>
              </div>
              <div className="text-end">
                <div className="font-bold">{match.broadcast}</div>
                <div>Watch on</div>
              </div>
            </div>
          </div>

          <div className="flex justify-between px-8 py-4 text-xs/tight font-bold">
