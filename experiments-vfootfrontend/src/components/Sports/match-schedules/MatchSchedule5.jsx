...
export default function MatchSchedule5() {
  return (
    <div className="mx-auto w-full max-w-[340px]">
      {data.map((match) => (
        <div
          key={match.id}
          className="@container overflow-hidden rounded-3xl border border-custom-gray-200 bg-custom-gray-100 text-gray-800 dark:border-custom-gray-600 dark:bg-custom-gray-700 dark:text-white"
        >
          <div className="bg-white py-3 text-center text-xs/tight font-bold dark:bg-custom-gray-800">{match.date}</div>
          <div className="relative isolate overflow-hidden rounded-b-3xl bg-white px-4 ring-1 ring-custom-gray-200 dark:bg-custom-gray-800 dark:ring-custom-gray-600">
            <div className="flex h-14 items-center justify-between">
              {match.teams.map((team) => (
                <div key={team.name} className="group flex items-center gap-x-3 text-sm/tight font-bold last:flex-row-reverse">
                  <div className="relative isolate h-14 w-12 shrink-0 overflow-hidden group-first:[clip-path:polygon(0_0,_100%_0,_100%_100%,_30%_100%)] group-last:[clip-path:polygon(0_0,100%_0,70%_100%,0_100%)]">
                    <svg
                      viewBox="0 0 420 420"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                      className={`absolute top-1/2 mt-1.5 aspect-square w-16 -translate-y-1/2 group-first:end-0 group-last:start-0 ${team.color.primary}`}
                    >
                      <path
                    ...
                    <div
                      className={`absolute inset-0 -z-10 group-first:[clip-path:polygon(0_0,_70%_0,_100%_100%,_30%_100%)] group-last:[clip-path:polygon(30%_0,100%_0,70%_100%,0_100%)] ${team.color.secondary}`}
                    ></div>
                  </div>
                  <div>{team.abbr}</div>
                </div>
          ...
          <div className="flex justify-between px-8 py-4 text-xs/tight font-bold">
            ...
            <div>
              <span>{match.time.start}</span>
              <span>{match.time.meridiem}</span>
              <span className="ml-0.5 font-normal">{match.time.zone}</span>
            </div>
          </div>
        </div>
      ))}
