...
export default function MatchSchedule4() {
  return (
    <div className="mx-auto w-full max-w-[340px]">
      {data.map((match) => (
        ...
        >
          <div className="flex items-center rounded-3xl bg-white ring-1 ring-custom-gray-200 dark:bg-custom-gray-800 dark:ring-custom-gray-600">
            <div className="flex-1 rounded-3xl p-8 ring-1 ring-custom-gray-200 dark:ring-custom-gray-600">
              <div className="flex flex-col gap-5">
                {match.teams.map((team) => (
                  <div key={team.abbr} className="flex gap-2 odd:order-1 even:order-3">
                    <figure>
                      <svg viewBox="0 0 420 420" fill="none" xmlns="http://www.w3.org/2000/svg" className={`h-8 w-8 ${team.color.primary}`}>
                        <path
                    ...
                    <div className="min-w-0 text-custom-gray-900 dark:text-white">
                      <div className="truncate text-sm/tight font-bold">{team.name}</div>
                      <div className="text-2xs/tight font-normal">{team.origin}</div>
                    </div>
                  </div>
                ))}
                <div className="order-2 flex items-center gap-x-2">
                  <div className="h-px w-8 bg-custom-gray-200 dark:bg-custom-gray-600"></div>
                  <div className="text-xs/tight font-bold text-custom-gray-900 dark:text-white">VS</div>
                  <div className="h-px flex-1 bg-custom-gray-200 dark:bg-custom-gray-600"></div>
                </div>
              </div>
            </div>
            <div className="px-7 text-end">
              <div className="text-sm/tight font-bold">{match.time.start}</div>
              <div className="text-xs/tight font-bold">{match.time.meridiem}</div>
              <div className="text-xs/tight">{match.time.zone}</div>
            </div>
          </div>
